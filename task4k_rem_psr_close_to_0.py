...
if f'wt_acc_{len(enus)}_bins_rem_psr_cl_0.pkl' in os.listdir(altier_path[0]):# or f'wt_acc_rem_psr_cl_0.pkl_{len(enus)}' in os.listdir(altier_path[1]):
    print("Loading wt_acc from pickle")
    # try:
    #     wt_acc = pickle.load(altier_path[1] + f'wt_acc_rem_psr_cl_0.pkl_{len(enus)//2}_bins')
    # except:
    with open(altier_path[0] + f'wt_acc_{len(enus)}_bins_rem_psr_cl_0.pkl', 'rb') as f:
    ...
    with open(altier_path[0] + f'wt_acc_{len(enus)}_bins_rem_psr_cl_0.pkl', 'wb') as f:
...
def Sik_sing_s_g(gamma_index, ws):#, wt_acc=wt_acc, w_models=w_models):
    ...
    return np.array(tmp, dtype=np.float64)
#Pickle
if os.path.isfile(altier_path[0] + f'all_Si_ws_g_s_{len(enus)}_bins_rem_psr_cl_0.pkl'):
    print("Loading all_Si_ws_g_s from pickle")
    with open(altier_path[0] + f'all_Si_ws_g_s_{len(enus)}_bins_rem_psr_cl_0.pkl', 'rb') as f:
    ...
    #Save to pickle
    with open(altier_path[0] + f'all_Si_ws_g_s_{len(enus)}_bins_rem_psr_cl_0.pkl', 'wb') as f:
...
#Pickle
if os.path.isfile(altier_path[0] + f'all_Bi_rem_psr_cl_0.pkl'):
    print("Loading all_Bi from pickle...")
    with open(altier_path[0] + f'all_Bi_rem_psr_cl_0.pkl', 'rb') as f:
    ...
    #Save to pickle
    with open(altier_path[0] + f'all_Bi_rem_psr_cl_0.pkl', 'wb') as f:
...
#Pickle
arr = []
if os.path.isfile(altier_path[0] + f'ns_all_ws_{len(enus)}_bins_rem_psr_cl_0.pkl'):
    print("Loading ns_hat from pickle...")
    with open(altier_path[0] + f'ns_all_ws_{len(enus)}_bins_rem_psr_cl_0.pkl', 'rb') as f:
        arr = pickle.load(f)
    ...
    with open(altier_path[0] + f'ns_all_ws_{len(enus)}_bins_rem_psr_cl_0.pkl', 'wb') as f:
        pickle.dump(arr, f)
...
with open(altier_path[0] + f'all_TSS_{len(enus)}_bins_rem_psr_cl_0.pkl', 'wb') as f:
...
plt.tight_layout()
plt.savefig(f'outputs/TS_vs_E2dfde_all_w_model_bins={len(enus)}_rem_psr_cl_0.png')
# plt.show()
print(f'\nTS_vs_E2dfde_all_w_model_bins={len(enus)}_rem_psr_cl_0.png\nDONE')
...
plt.tight_layout()
plt.savefig(f'outputs/UL_all_w_model_bins={len(enus)}_rem_psr_cl_0.png')
plt.show()
...
#     plt.savefig(os.getcwd() + f'/outputs/TSvsE2dFdE_ws_{i}_rem_psr_cl_0.png', facecolor='w')
#     plt.show()
