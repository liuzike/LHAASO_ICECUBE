...
REMOVED_SEASON = np.array([0, 1, 2,  6, 7, 8, 9])
seasons = list(np.arange(0, 10, 1))
seasons.remove(1)
seasons.remove(2)
seasons = np.asfortranarray(seasons, dtype=np.int64)
...
icra_rm_s = np.delete(np.arange(0, 1134450), np.arange(season_walls[REMOVED_SEASON[0]], season_walls[REMOVED_SEASON[0]+1]))
icra_rm_s2 = np.delete(np.arange(0, 1134450), np.arange(season_walls[REMOVED_SEASON[1]], season_walls[REMOVED_SEASON[1]+1]))
icra_rm_s = list(set(icra_rm_s).intersection(icra_rm_s2))
icra_rm_s.sort()
icra_rm_s = np.asarray(icra_rm_s)
# print(30960 in icra_rm_s, 38960 in icra_rm_s, 125360 in icra_rm_s, 160960 in icra_rm_s, 224560 in icra_rm_s, 360960 in icra_rm_s)
# exit()
#Pickle
if os.path.isfile(altier_path[0] + f'all_Si_ws_g_s_{len(enus)}_bins_rm_s_{REMOVED_SEASON}.pkl'):
    print("Loading all_Si_ws_g_s from pickle")
    with open(altier_path[0] + f'all_Si_ws_g_s_{len(enus)}_bins_rm_s_{REMOVED_SEASON}.pkl', 'rb') as f:
    ...
    #Save to pickle
    with open(altier_path[0] + f'all_Si_ws_g_s_{len(enus)}_bins_rm_s_{REMOVED_SEASON}.pkl', 'wb') as f:
...
#Pickle
if os.path.isfile(altier_path[0] + f'all_Bi_rm_s_{REMOVED_SEASON}.pkl'):
    print("Loading all_Bi from pickle...")
    with open(altier_path[0] + f'all_Bi_rm_s_{REMOVED_SEASON}.pkl', 'rb') as f:
    ...
    #Save to pickle
    with open(altier_path[0] + f'all_Bi_rm_s_{REMOVED_SEASON}.pkl', 'wb') as f:
...
with open(altier_path[0] + f'all_TSS_{len(enus)}_bins_rm_s_{REMOVED_SEASON}.pkl', 'wb') as f:
