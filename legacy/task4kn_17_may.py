...
num_threads = int(mul.cpu_count()*0.96)
...
def psr_wt_sing_gamma(psrno,gamma, season):
    '''
    Calculates the weights for a single pulsar for a single season for a single gamma
    '''
...
def ns_singleseason_sing_psr_HAT(psrno,gamma, phi0, season):
    ...
    for i in prange(len(enus)):
        ns_temp[i] += np.float64(np.sin(np.deg2rad(msdec[psrno])) * tt_upt * earea[ea_season(season)][l*40 + enus_bin_indices[i]] * phi0 * (enus[i]/(10**14))**gamma)
...
axs[0].set_title('Weighting scheme: $w_{acc}$; $w_{model} = 1$', fontdict=smallerfont)
axs[1].set_title('Weighting scheme: $w_{acc}$; $w_{model} = \dfrac{1}{d_{DM}^2}$' , fontdict=smallerfont)
axs[2].set_title('Weighting scheme: $w_{acc}$; $w_{model} = s_{1400}$', fontdict=smallerfont)
...
for i in range(3):
    ...
    axs[i].set_xlabel('$E^2_{\u03BD} \dfrac{dF}{dE_{\u03BD}}$ at 100 TeV ($GeV$ $s^{-1}$ $cm^{-2}$ $sr^{-1}$)', fontdict=axesfont)
...
for i in range(3):
    ...
    axs[i].set_ylabel("$E^2_{\u03BD} \dfrac{dF}{dE_{\u03BD}}$ (GeV $s^{-1}$ $cm^{-2}$ $sr^{-1}$)", fontdict=axesfont)
    axs[i].xaxis.set_tick_params(labelsize=15)
    axs[i].yaxis.set_tick_params(labelsize=15)
    
    axs[i].legend(prop={'size':14}, framealpha=0, loc='center left')
