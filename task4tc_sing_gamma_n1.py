...
for cyc in prange(n_cyc):
    ...
    altier_path = [os.getcwd() + f'/pickle/node{half_name}_sg/', os.getcwd() + '/../pickle/']
    ...
    # if os.path.isfile(altier_path[0] + f'all_Si_ws_g_s_{len(enus)}_bins_C_wt_all_psr_wmod1_btsp4_gamma253_d_off_{dec_offset}_cyc_{cyc}.pkl'):
    #     print("Loading all_Si_ws_g_s from pickle")
    #     with open(altier_path[0] + f'all_Si_ws_g_s_{len(enus)}_bins_C_wt_all_psr_wmod1_btsp4_gamma253_d_off_{dec_offset}_cyc_{cyc}.pkl', 'rb') as f:
    ...
    print("Calculated S_i for all neutrinos and gammas and weighting schemes")
        #Save to pickle
    with open(altier_path[0] + f'all_Si_ws_g_s_{len(enus)}_bins_C_wt_all_psr_wmod1_btsp4_gamma253_d_off_{dec_offset}_cyc_{cyc}.pkl', 'wb') as f:
    ...
    #Pickle
    arr = []
    # if os.path.isfile(altier_path[0] + f'ns_all_ws_{len(enus)}_bins_C_wt_all_psr_wmod1_btsp4_gamma253_d_off_{dec_offset}_cyc_{cyc}.pkl'):
    #     print("Loading ns_hat from pickle...")
    #     with open(altier_path[0] + f'ns_all_ws_{len(enus)}_bins_C_wt_all_psr_wmod1_btsp4_gamma253_d_off_{dec_offset}_cyc_{cyc}.pkl', 'rb') as f:
    #         arr = pickle.load(f)
    ...
    with open(altier_path[0] + f'ns_all_ws_{len(enus)}_bins_C_wt_all_psr_wmod1_btsp4_gamma253_d_off_{dec_offset}_cyc_{cyc}.pkl', 'wb') as f:
        pickle.dump(arr, f)
    ...
    with open(altier_path[0] + f'all_TSS_{len(enus)}_bins_C_wt_all_psr_wmod1_btsp4_gamma253_d_off_{dec_offset}_cyc_{cyc}.pkl', 'wb') as f:
    ...
    for gamma in range(len(gamma_arr)):#range(4):
    ...
    plt.tight_layout()
    plt.savefig(f'outputs/TS_vs_E2dfde_all_w_model_bins={len(enus)}_btsp4_gamma253_d_off_{dec_offset}_cyc_{cyc}.pdf')
    # plt.show()
    print(f'\nTS_vs_E2dfde_all_w_model_bins={len(enus)}_btsp4_gamma253_d_off_{dec_offset}_cyc_{cyc}.pdf\nDONE')
    
    print(f'{n_cyc} cycle COMPLETED')
